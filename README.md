...
- **登录闭环**：面板点「登录账号」→ 浏览器登录 → 回跳面板自动落盘（异机部署可粘贴兜底）；`login.sh` 走命令行粘贴
...
（同一件事在面板里更省事：点「登录账号」登录完自动回跳落盘，不用粘贴，见「管理面板」。）
...
可签到、刷新剩余积分、禁用、解除冷却、移除，以及添加账号后立刻写入 `auths/trae-{uid}.json` 并进池，不用重启。

「登录账号」拿到授权链接，浏览器登录完成后 TRAE 会跳回**面板自己的回调地址**，账号自动落盘、无需粘贴
（粘贴框保留作退路：回跳不通时照样能换票）。回跳地址 = `login.callback_url` + `/panel/oauth/callback/<一次性 id>`：
`login.callback_url` 留空按 `listen` 拼 `http://127.0.0.1:7864`（面板与浏览器同机时直接可用）；
**异机部署**（面板在服务器、浏览器在你自己电脑上）填你那台浏览器能打开的面板地址，例如 `https://panel.example.com`，
否则 `127.0.0.1` 指的是浏览器那台机器，回跳必然落空。保存后立即生效。
这条回跳路由**故意不鉴权**（浏览器 302 过来带不了 `Authorization` 头），兜底是地址里的一次性 id：
只有刚点过登录的那次会话认得它，15 分钟过期、成功即删；页面只回文案，不回显任何 token。
